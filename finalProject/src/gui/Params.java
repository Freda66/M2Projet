...
public class Params extends javax.swing.JFrame {
    ...
    private void run() {
        ...
        // Reset db
        Result db = new Result(new Database("./database.db"));
        db.clearResult();

        // Inititialisation du timestamp
        Date date = new Date();
        Timestamp timeInit = new Timestamp(date.getTime());

        // Debut de la run
        Runner run = new Runner(db);
        run.setTimeIn(timeInit);

        // ----- Intégration des différentes parties du programme -----
        PVirg arbre = null;

        // Création de l'arbre après analyse du fichier C
        CParser tree = new CParser();
        arbre = tree.parseIt();

        // Application de l'algorithme SSA
        CmdFormatter formatter = new CmdFormatter();
        arbre = (PVirg) formatter.toSSA(arbre);

        // Test sur le fichier source avec enregistrement des données dans db
        // String nameFile, String dir, PVirg t, String cheminParams
        Ermes ermes = new Ermes("code.c", "/", arbre, "/");
        ermes.RunErmes();

        // Parcour de l'arbre avec appel de l'optimisateur d'exression
        formatter.transformExpressions(arbre);

        // Test sur le fichier optimisé avec enregistrement des données dans db
        ermes.RunErmesOpt(arbre);
        
        // ----- Fin Intégration -----
        // Fin de la run
        Timestamp timeEnd = new Timestamp(date.getTime());
        run.setTimeOut(timeEnd);
    }
